...
function createHeavyZodSchema() {
    const DEPTH = 100;
    const KEYS_PER_LEVEL = 100;

    let current = z.string().min(5);
    ...
    for (let i = 0; i < DEPTH; i++) {
        ...
        for (let j = 0; j < KEYS_PER_LEVEL; j++) {
            props[`level_${i}_key_${j}`] = z.number().optional();
        }
        current = z.object(props);
    }
...
async function runBenchmark() {
    console.log('--- Zod Performance Test ---');
    console.log('Generating heavy Zod schema (100x100)...');

    const memStart = process.memoryUsage().heapUsed;
    const start = performance.now();
    const schema = createHeavyZodSchema();
